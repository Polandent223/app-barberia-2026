...
import {uploadImages,downloadImages,enableImageRealtime,disableImageRealtime,cloudImageStatus} from "./firebase-images.js";
...
function controls(){
  ...
  ["firebaseUploadBtn","firebaseDownloadBtn","firebaseRealtimeBtn","firebaseUploadImagesBtn","firebaseDownloadImagesBtn"].forEach(id=>{if($(id))$(id).disabled=!on});
...
async function logout(){try{disableRealtime();disableImageRealtime();Bridge.realtime=false;await firebaseLogout();window.App?.toast?.("Firebase desconectado")}catch(e){Bridge.setError(e)}}
...
function realtime(){if(Bridge.realtime){disableRealtime();disableImageRealtime();Bridge.realtime=false;window.App?.toast?.("Tiempo real desactivado")}else{enableRealtime();enableImageRealtime();Bridge.realtime=true;window.App?.toast?.("Tiempo real activo")}controls()}
...
async function uploadPics(){
  ...
  try{
    await uploadImages();
    Bridge.setSynced();
    const s=cloudImageStatus();
    window.App?.toast?.(`Imágenes subidas: ${s.barberCount} barberos`);
...
async function downloadPics(){
  ...
  try{
    ...
    Bridge.setSynced();
    window.App?.toast?.("Imágenes descargadas");
  }catch(e){Bridge.setError(e)}
}
