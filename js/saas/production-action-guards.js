...
(function(){
  ...
  const cloudReady=()=>{if(!window.FirebaseBridge?.connected)return true;if(role()==='superadmin')return true;const id=String(S?.getContext?.()?.businessId||S?.session?.businessId||'').trim(),ready=window.SaaSCloudProduction?.isTenantReady;if(!id||typeof ready!=='function')return false;return ready(id)===true};
  ...
  wrap('editAppointment',{page:'citas',domain:'schedule',before:function(id){const a=(A.db?.appointments||[]).find(x=>x.id===id);if(a?.status==='Finalizada')return deny('Una cita finalizada queda bloqueada para proteger el cobro y el recibo')}});wrap('deleteAppointment',{page:'citas',domain:'schedule'});wrap('confirmAppointment2032',{page:'citas',domain:'schedule'});wrap('rejectAppointment2032',{page:'citas',domain:'schedule'});wrap('approveClientRequest',{page:'citas',domain:'schedule'});wrap('rejectClientRequest',{page:'citas',domain:'schedule'});
  ...
  wrap('saveClient',{page:'clientes',domain:'crm'});wrap('deleteClient',{page:'clientes',domain:'crm'});wrap('saveService',{page:'servicios',domain:'config'});wrap('deleteService',{page:'servicios',domain:'config'});
  // inventory.js modifica stock mediante stockMove; se protege el punto de entrada real además de aliases futuros.
  wrap('saveProduct',{page:'inventario',domain:'inventory'});wrap('editProduct',{page:'inventario',domain:'inventory'});wrap('deleteProduct',{page:'inventario',domain:'inventory'});wrap('stockMove',{page:'inventario',domain:'inventory'});wrap('addStockMove',{page:'inventario',domain:'inventory'});
