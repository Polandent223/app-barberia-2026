...
function hydrateTenant(id,{createIfMissing=false}={}){
  ...
  const generation=sessionGeneration,uid=String(window.FirebaseBridge?.user?.uid||"");
  const task=(async()=>{
    refreshHydrationUI();
    const got=await downloadTenant(id);
    if(generation!==sessionGeneration||uid!==String(window.FirebaseBridge?.user?.uid||""))return false;
    if(!got&&createIfMissing){if(currentBusinessId()!==id)throw new Error("El negocio cambió durante la sincronización inicial.");await uploadCurrentTenant()}
    if(generation!==sessionGeneration||uid!==String(window.FirebaseBridge?.user?.uid||""))return false;
    hydratedTenants.add(id);if(currentBusinessId()===id)watchCurrentTenant();return got;
  })().finally(()=>{if(hydrationPromises.get(id)===task)hydrationPromises.delete(id);refreshHydrationUI()});
...
function installHooks(){
  ...
  const A=window.App,oldPersist=A.persist.bind(A);A.persist=function(){const id=currentBusinessId(),role=SaaS.session?.role;if(window.FirebaseBridge?.connected&&id&&tenantHydrating(id)){A.toast?.("Espera unos segundos: SAMBRIX todavía está sincronizando este negocio.");return false}const r=oldPersist();if(!window.FirebaseBridge?.connected||!["owner","admin","manager","reception","cashier","barber","superadmin"].includes(role))return r;if(id&&!hydratedTenants.has(id)&&role!=="superadmin"){hydrateTenant(id,{createIfMissing:true}).catch(console.error);return r}uploadCurrentTenant().then(()=>{if(["owner","admin","manager","superadmin"].includes(SaaS.session?.role))return window.NexoPublicCloud?.publishCurrentBusiness?.()}).catch(console.error);return r};
  hooked=true;
}

async function syncForCurrentSession(){
  if(!window.FirebaseBridge?.connected||!window.SaaS)return false;let role=SaaS.session?.role||"guest";if(role==="guest"){await SaaS.resolveFirebaseSession?.();role=SaaS.session?.role||"guest"}if(role==="guest")return false;
  installHooks();
  if(role==="superadmin"){await cloudBootstrapPlatform();await window.SaaSAuthAdmin?.refreshAccess?.();const got=await downloadBusinessCatalog();if(!got)await queueCatalogUpload();await window.SaaSAuthAdmin?.repairBusinessUserLinks?.();await ensureCatalogTenants();watchCatalog();return true}
  const businessId=String(SaaS.session?.businessId||SaaS.getContext?.()?.businessId||"").trim();if(!businessId)return false;if(SaaS.getContext?.()?.businessId!==businessId)SaaS.switchTenant?.(businessId,{support:false});await hydrateTenant(businessId,{createIfMissing:true});return true;
}
