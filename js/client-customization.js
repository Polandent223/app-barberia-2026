...
App.setBarberPhoto=function(id,file){
  App.fileToDataUrl(file,data=>{App.db.business.clientApp.barberPhotos[id]=data;App.renderBarberPhotoEditor();App.toast("Foto actualizada")});
};
...
App.saveClientCustomization=function(){
  ...
  if(logo){pending++;App.fileToDataUrl(logo,d=>{c.logo=d;done()})}
  if(bg){pending++;App.fileToDataUrl(bg,d=>{c.background=d;done()})}
