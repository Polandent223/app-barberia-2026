...
App.saveProduct = function(){
  ...
  const data={
    name:App.val("productName"),
    category:App.val("productCategory"),
    stock:Number(App.val("productStock")||0),
    min:Number(App.val("productMin")||0),
    cost:Number(App.val("productCost")||0),
    price:Number(App.val("productPrice")||0)
  };
  const finish=photo=>{
    if(existing){
      ...
      App.db.products.push({id:App.uid(),...data,photo:photo||""});
    }
  ...
  if(file)App.compressImageLocal(file,720,.8).then(finish).catch(()=>{App.toast("No se pudo procesar la foto");finish("")});
  else finish("");
};
App.editProduct = function(id){
  const p=App.db.products.find(x=>x.id===id);if(!p)return;
  App.show("productForm");
  App.byId("productEditId").value=p.id;
  App.byId("productName").value=p.name||"";
  App.byId("productCategory").value=p.category||"Insumo";
  App.byId("productStock").value=p.stock??0;
  App.byId("productMin").value=p.min??0;
  App.byId("productCost").value=p.cost??0;
  App.byId("productPrice").value=p.price??0;
  const preview=App.byId("productPhotoPreview");
  if(preview){
    preview.src=p.photo||"";
    preview.classList.toggle("hidden",!p.photo);
  }
